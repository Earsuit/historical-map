function(createTagVersion)
    ...
    find_package(Git)
    if (GIT_FOUND)
        execute_process(COMMAND ${GIT_EXECUTABLE} describe --tags --dirty --match "v[0-9]*\\.[0-9]*\\.[0-9]*" OUTPUT_VARIABLE FULL_TAG_VERSION)
        message(STATUS "Tag version ${FULL_TAG_VERSION}")
        string(REGEX REPLACE "^v([0-9]+)\\..*" "\\1" VERSION_MAJOR "${FULL_TAG_VERSION}")
        string(REGEX REPLACE "^v[0-9]+\\.([0-9]+).*" "\\1" VERSION_MINOR "${FULL_TAG_VERSION}")
        string(REGEX REPLACE "^v[0-9]+\\.[0-9]+\\.([0-9]+).*" "\\1" VERSION_PATCH "${FULL_TAG_VERSION}")
        ...
        if ("${VERSION_MAJOR}" STREQUAL "" OR "${VERSION_MINOR}" STREQUAL "" OR "${VERSION_PATCH}" STREQUAL ""
            ...
            set(VERSION_MAJOR "v0" PARENT_SCOPE)
